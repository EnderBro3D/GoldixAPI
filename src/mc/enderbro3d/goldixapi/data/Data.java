...
public class Data {
    ...
    public void save(boolean async) {
        Runnable r = () -> {
            ...
            MySQLWorker.execute(async,"INSERT INTO `goldix`.`bedwars_stats`(`name`, `kills`, `deaths`, `wins`, `played`) VALUES(?, ?, ?, ?, ?) " +
            ...
            MySQLWorker.execute(async,"INSERT INTO `goldix`.`skywars_stats`(`name`, `kills`, `deaths`, `wins`, `played`) VALUES(?, ?, ?, ?, ?) " +
            ...
            System.out.println(table.contains(GameType.GLOBAL, GlobalValueType.LANG));
            int lang = getData(GameType.GLOBAL, GlobalValueType.LANG).integerValue();

            MySQLWorker.execute(async,"INSERT INTO `goldix`.`userdata`(`name`, `group`, `level`, `exp`, `balance`, `lang`) VALUES(?, ?, ?, ?, ?, ?) " +
    ...
    public void load(boolean async) {
        Runnable r = () -> {
            ...
            try {
                //Bedwars Loading
                try (ResultSet rs = MySQLWorker.executeQuery(async,"SELECT * FROM `goldix`.`bedwars_stats` WHERE `name`=?", name)) {
                ...
                //Skywars Loading
                try (ResultSet rs = MySQLWorker.executeQuery(async,"SELECT * FROM `goldix`.`skywars_stats` WHERE `name`=?", name)) {
                    if (MySQLWorker.next(rs)) {
                ...
                //UserData loading
                try (ResultSet rs = MySQLWorker.executeQuery(async,"SELECT * FROM `goldix`.`userdata` WHERE `name`=?", name)) {
                    if (MySQLWorker.next(rs)) {
                        ...
                        table.put(GameType.GLOBAL, GlobalValueType.LANG, MySQLWorker.get(rs, "lang"));
                        ...
                        table.put(GameType.GLOBAL, GlobalValueType.LANG, new Value(0));
                    }
                }
                save(async);
